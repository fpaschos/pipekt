...
class PipelineDefinitionTest : FunSpec({
    ...
    test("valid pipeline builds successfully") {
        val result = pipeline<String>("test-pipeline", maxInFlight = 100) {
    ...
    test("retentionDays defaults to 30") {
        val result = pipeline<String>("default-retention", maxInFlight = 10) {
            source("src", fakeAdapter())
            step<String, String>("step1") { it }
        }
    ...
    test("retentionDays can be overridden") {
        val result = pipeline<String>("custom-retention", maxInFlight = 10, retentionDays = 7) {
            source("src", fakeAdapter())
            step<String, String>("step1") { it }
        }
    ...
    test("duplicate step names are rejected") {
        val result = pipeline<String>("dupe-test", maxInFlight = 10) {
            source("src", fakeAdapter())
    ...
    test("source name clashing with step name is rejected") {
        val result = pipeline<String>("clash-test", maxInFlight = 10) {
