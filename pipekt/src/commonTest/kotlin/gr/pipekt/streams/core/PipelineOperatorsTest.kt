...
class PipelineOperatorsTest : FunSpec({

    fun fakeAdapter() = object : SourceAdapter<String> {
    ...
    test("PersistEachDef name is preserved") {
        val result = pipeline<String>("persist-test", maxInFlight = 10) {
            source("src", fakeAdapter())
            step<String, String>("step1") { it }
            persistEach("checkpoint-1")
        }
        result.isRight() shouldBe true
        val persistOp = result.getOrNull()!!.operators.filterIsInstance<PersistEachDef>().first()
        persistOp.name shouldBe "checkpoint-1"
    }

    test("operator list order is retained") {
        val result = pipeline<String>("order-test", maxInFlight = 10) {
            source("src", fakeAdapter())
    ...
    test("source definition is preserved in pipeline") {
        ...
        val result = pipeline<String>("src-test", maxInFlight = 10) {
    ...
    test("pipeline with filter step and persistEach builds successfully") {
        val result = pipeline<String>("full-pipeline", maxInFlight = 50) {
            source("src", fakeAdapter())
